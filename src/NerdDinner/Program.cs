...
// M11 (decision-log.md DL-031): the legacy Framework app and the YARP
// reverse proxy that sat in front of it (M7, DL-020) are both gone --
// every controller (Home, Dinners, RSVP, Search, Account) has been
// migrated here across M8-M10, so this is now the sole application, not
// one half of a strangler-fig pair. The controller-name constraint
// M8-M10 needed on this route (to leave unmigrated controllers falling
// through to the proxy) no longer serves a purpose; back to the
// standard, unconstrained default MVC route.
...
// NerdDinner.Tests, which hosts this app for real via
