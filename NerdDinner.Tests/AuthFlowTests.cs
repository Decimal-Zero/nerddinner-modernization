...
{
    // Category=Integration: not because of a live external dependency
    // (that reason -- needing the legacy app running -- went away at
    // M11, decision-log.md DL-031) but because this is the one test
    // class that writes to the shared dev "NerdDinner" database
    // (NerdDinnerCoreContext isn't overridden to a test-only database
    // the way ApplicationDbContext is, below) -- kept out of the default
    // fast run so ordinary test cycles don't depend on or perturb
    // whatever's in that database.
    //
    // Real end-to-end verification of M10's acceptance criteria
    // (plan.md) -- "session handling across the proxy boundary during
    // the transition period ... explicitly tested, not assumed." Uses a
    // real HttpClient with cookie handling enabled, through the real
    // ASP.NET Core Identity + cookie authentication pipeline -- this is
    // specifically testing that the cookie Account/Register issues is
    // honored by a completely different, [Authorize]-gated controller
    ...
    // IdentityTestDatabaseFixture) so this suite's throwaway
    ...
    public class AuthFlowTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        public AuthFlowTests(IdentityTestDatabaseFixture identityFixture, WebApplicationFactory<Program> factory)
        {
            _factory = factory.WithWebHostBuilder(builder =>
            {
                builder.ConfigureTestServices(services =>
                {
                    ...
                    services.AddDbContext<ApplicationDbContext>(options =>
                        options.UseSqlServer(IdentityTestDatabaseFixture.ConnectionString));
                });
            });
        }
